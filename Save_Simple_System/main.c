...
//    printf("62\n");
    for(i = 0; i < FILES; i++)
    {
        for(j = 0; j < STRIDES; j++)
        {
            time_classifiers[i][j] = (float *) malloc(sizeof ( float) * TIME_FEATURES);
            memset(time_classifiers[i][j], 0, sizeof(float) * TIME_FEATURES);
            time_speeds[i][j] = (float *) malloc(sizeof (float) * TIME_SPEED_FEATURES);
            memset(time_speeds[i][j], 0, sizeof(float) * TIME_SPEED_FEATURES);
            time_turn_speeds[i][j] = (float *) malloc(sizeof (float) * TIME_TURN_SPEED);
            memset(time_turn_speeds[i][j], 0, sizeof(float) * TIME_TURN_SPEED);
            time_turning_all[i][j] = (float *) malloc(sizeof (float) * TIME_TURN_FEATURES);
            memset(time_turning_all[i][j], 0, sizeof(float) * TIME_TURN_FEATURES);
        }
    }
...
//    build names of .csc .txt and .net files for data, training and testing.
    ...
    sprintf(time_dot_net_turning, "%s%s_%s/time_%s_%s.net", main_path, train_file, name, turning_name, name);
    sprintf(time_dot_text_turning,"%s%s_%s/time_%s_%s.txt", main_path, train_file, name, turning_name, name);
    ...
    // path, folder user
    sprintf(time_dot_net_name[0], "%s%s_%s/time_%s_%s.net", main_path, train_file, name, activities_file, name);
    sprintf(time_dot_text_names[0],"%s%s_%s/time_%s_%s_%s", main_path, train_file, name, activities_file, name, train_name);


    for (i = 0; i < ACTIVITIES; i++) {
        sprintf(time_dot_net_name[i+1], "%s%s_%s/time_%s_%s.net", main_path, train_file, name, train_speeds[i], name);
        sprintf(time_dot_text_names[i+1],"%s%s_%s/time_%s_%s_%s", main_path, train_file, name, train_speeds[i], name, train_name);
        // now lists of .net and txt names are ready
    }
    for (i = 0; i < NUM_TURNS; i++) {
        time_dot_text_turn_name[i] = (char *) malloc(sizeof(char) * BUFF_SIZE);
        memset(time_dot_text_turn_name[i], 0, sizeof(char) * BUFF_SIZE);
        sprintf(time_dot_text_turn_name[i], "%s%s_%s/time_%s_%s.txt", main_path, train_file, name, train_turn_speeds[i], name);

        time_dot_net_turn_speeds[i] = (char *) malloc(sizeof(char) * BUFF_SIZE);
        memset(time_dot_net_turn_speeds[i], 0, sizeof(char) * BUFF_SIZE);
        sprintf(time_dot_net_turn_speeds[i], "%s%s_%s/time_%s_%s.net", main_path, train_file, name, train_turn_speeds[i], name);
    }
...
//            printf("i: %d\t", i);
            ...
            features(data[i], mean[i], sigma[i], strides_t2[i], s_tr[i] +1,
                     classifiers[i], turning_all[i], speeds[i], turn_speeds[i], &dimen[i]);

            time_features(data[i], mean[i], sigma[i], strides_t2[i], s_tr[i] +1,
                     time_classifiers[i], time_turning_all[i], time_speeds[i], time_turn_speeds[i], &dimen[i]);
        }
        for(i = PRINT; i < FILES; i++)
        {
            for(j = 0; j < s_tr[i] && j < dimen[i]; j++)
            {
                printf("%s, up to: %d, max: %d\n", names[i], s_tr[i], dimen[i]);
                for(k = 0; k < FEATURES; k++)
                {
                    printf("%f\t", turning_all[i][j][k]);
...
//        #define TIME_TURN_SPEED 400
        ...
        training_file(time_classifiers, time_dot_text_names[0], dimen, activities, TIME_FEATURES, 0, NO_TURN, ACTIVITIES);

        turning_file(time_turning_all, time_dot_text_turning, dimen, TIME_TURN_FEATURES);
        for (i = 0; i < ACTIVITIES; i++) {
            speeds_count = speeds_end[i] - speeds_start[i];
            printf("\n%s speeds: %d\n", dot_text_names[i+1], speeds_count);
            training_file(time_speeds, time_dot_text_names[i+1], dimen, speeds_outputs, TIME_SPEED_FEATURES, speeds_start[i], speeds_end[i],
                          speeds_count);
        ...
        for (i = 0; i < NUM_TURNS; i++)
        {
            speeds_count = speeds_end[i+ACTIVITIES] - speeds_start[i+ACTIVITIES];
            printf("\n%s speeds: %d\n", dot_text_turn_name[i], speeds_count);
            training_file(time_turn_speeds, time_dot_text_turn_name[i], dimen, speeds_outputs,
                          TIME_TURN_SPEED, speeds_start[i+ACTIVITIES], speeds_end[i+ACTIVITIES], speeds_count);
        }
