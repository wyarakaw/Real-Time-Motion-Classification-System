...
void time_create_names(char *name, char ***c, char **turn, char ***turn_speed){
    char **dot_net_name;
...
"home/directory"
*/
void time_test_neural_network(float **class_v, float **turning, float **speeds,
...
//    sprintf(TURNING_RIGHT,"%sTURNING_RIGHT_%s.net", net_files_directory, name);
    ...
    sprintf(TRAINING, "time_%s_%s.net", activities_file, name);
    sprintf(WALKING, "time_%s_%s.net", train_speeds[0], name);
    sprintf(RUNNING, "time_%s_%s.net", train_speeds[1], name);
    sprintf(STAIR_ASCENT, "time_%s_%s.net", train_speeds[2], name);
    sprintf(STAIR_DESCENT, "time_%s_%s.net", train_speeds[3], name);
    sprintf(TWO_STAIR_ASCENT, "time_%s_%s.net", train_speeds[4], name);
    sprintf(TWO_STAIR_DESCENT, "time_%s_%s.net", train_speeds[5], name);
    sprintf(JUMPING, "time_%s_%s.net", train_speeds[6], name);

    sprintf(TURNING_ANN, "time_%s_%s.net", turning_name, name);

    sprintf(WALK_TURN_RIGHT, "time_%s_%s.net", train_turn_speeds[0], name);
    sprintf(WALK_TURN_LEFT, "time_%s_%s.net", train_turn_speeds[1], name);
    sprintf(RUN_TURN_RIGHT, "time_%s_%s.net", train_turn_speeds[2], name);
    sprintf(RUN_TURN_LEFT, "time_%s_%s.net", train_turn_speeds[3], name);
    sprintf(JUMP_TURN_RIGHT, "time_%s_%s.net", train_turn_speeds[4], name);
    sprintf(JUMP_TURN_LEFT, "time_%s_%s.net", train_turn_speeds[5], name);
    ...
    for (i = 0; i < num_rows_classifiers; i++)
    {
        ...
        switch (motion) {
            case 0:
                ...
                printf("\n");
                printf("%f\t%f\t%f\t", max, speed_max,  turn_max);
            ...
            case 6:
                ...
                if(turn == 0){
                    ...
                    printf("Motion: %s --> Height: %s\tTurn: %s %s\n", activities_output[motion],
                           walk_speeds[speed], turn_output[turn], jump_turn_output[turning_speed]);
                }
                break;
            default:
                break;
